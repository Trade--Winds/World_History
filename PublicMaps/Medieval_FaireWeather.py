##############################################################################
## File: FaireWeather.py version 1.04
...
## GLOBAL TUNING VARIABLES: Change these to customize the map results
        ...
        self.chanceForTreelessMarsh = 0.6
...
#CHANGED FROM 90 90
        ...
        self.horseLattitude = 23
        self.polarFrontLattitude = 60

        #Tropics of Cancer and Capricorn plus and minus respectively
        self.tropicsLattitude = 23
        ...
        self.northWaterBand = 20
        self.southWaterBand = 20
...
def getGridSize(argsList):
    ...
    grid_sizes = {
            WorldSizeTypes.WORLDSIZE_TINY:		(20 + mc.distanceToEurope * 2,30),
            WorldSizeTypes.WORLDSIZE_SMALL:		(25 + mc.distanceToEurope * 2,40),
            WorldSizeTypes.WORLDSIZE_STANDARD:	(30 + mc.distanceToEurope * 2,50),
            WorldSizeTypes.WORLDSIZE_LARGE:		(35 + mc.distanceToEurope * 2,60),
            WorldSizeTypes.WORLDSIZE_HUGE:		(45 + mc.distanceToEurope * 2,75)
    }
...
def afterGeneration():   
    ...
    for y in range(4):
        for x in range(mc.width):
            plot = mmap.plot(x,y)
            if plot != 0 and plot.isWater() == True and PRand.random() < iceChance:
                iRandTerrain = CyGame().getSorenRandNum(100, "FW RT")
                if (iRandTerrain > 85):
                    plot.setTerrainType(terrainTundra, True, True)
                elif (iRandTerrain > iHilLPeakChance):
                    iHillTerrain = CyGame().getSorenRandNum(100, "FW RT")
                    if (iHillTerrain > 75):
                        plot.setTerrainType(terrainTundra, True, True)
                        plot.setPlotType(PlotTypes.PLOT_HILLS,True,True)
                    else:
                    ...
                    plot.setTerrainType(terrainSnow, True, True)
