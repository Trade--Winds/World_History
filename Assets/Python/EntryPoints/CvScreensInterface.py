...
#TK INVENTIONS
import CvTechnologyAdvisor
import CvTradeAdvisor
import CvSpiceRouteScreen
import CvSilkRoadScreen
import CvTradeFairScreen
import CvImmigrationScreen
import CvEuroZoneScreen
#TKend
...
#TK MED INVENTIONS
technologyAdvisor = CvTechnologyAdvisor.CvTechnologyAdvisor()
def showTechnologyAdvisor(argsList):
	if (-1 != CyGame().getActivePlayer()):
		technologyAdvisor.interfaceScreen()
		
tradeAdvisor = CvTradeAdvisor.CvTradeAdvisor()
def showTradeAdvisor(argsList):
	if (-1 != CyGame().getActivePlayer()):
		tradeAdvisor.interfaceScreen(argsList[0])
		
spicerouteScreen = CvSpiceRouteScreen.CvSpiceRouteScreen()
def showSpiceRouteScreen(argsList):
	if (-1 != CyGame().getActivePlayer()):
		spicerouteScreen.interfaceScreen()
		
silkroadScreen = CvSilkRoadScreen.CvSilkRoadScreen()
def showSilkRoadScreen(argsList):
	if (-1 != CyGame().getActivePlayer()):
		silkroadScreen.interfaceScreen()

tradefairScreen = CvTradeFairScreen.CvTradeFairScreen()
def showTradeFairScreen(argsList):
	if (-1 != CyGame().getActivePlayer()):
		tradefairScreen.interfaceScreen()
...
eurozoneScreen = CvEuroZoneScreen.CvEuroZoneScreen()
def showEuroZoneScreen(argsList):
	if (-1 != CyGame().getActivePlayer()):
		eurozoneScreen.interfaceScreen()
#TKe 
...
HandleMouseoverMap = {  EUROPE_SCREEN : europeScreen,
				...
				EURO_ZONE_SCREEN: eurozoneScreen,
