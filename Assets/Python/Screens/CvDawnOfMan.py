...
class CvDawnOfMan:
	...
	def interfaceScreen(self):
		...
		screen.addDrawControl("DawnBackGroundPanel", ArtFileMgr.getInterfaceArtInfo("INTERFACE_ROYAL_CHARTER_BG").getPath(), self.X_MAIN_PANEL - 50, self.Y_MAIN_PANEL - 50, self.W_MAIN_PANEL + 100, self.H_MAIN_PANEL + 150, WidgetTypes.WIDGET_GENERAL, -1, -1 )
		...
		#TKs Med
		screen.setButtonGFC("Exit", self.EXIT_TEXT, "", self.X_EXIT, self.Y_EXIT + self.WH_FANCY_ICON + 10, self.W_EXIT, self.H_EXIT, WidgetTypes.WIDGET_CLOSE_SCREEN, -1, -1, ButtonStyles.BUTTON_STYLE_STANDARD )
		#TKe
		# Top
		...
		iNumUnits = self.player.getNumUnits()
		IconSize = self.WH_FANCY_ICON
		yUnitMod = 0
		iUnitCount = 0
		if iNumUnits > 3:
		...
		while(unit):
			if iUnitCount == 3:
				yUnitMod += 1
				iUnitCount = 0
			screen.addDDSGFC("Unit Icon" + str(unit.getID()), unit.getButton(), self.X_LEADER_ICON + (self.WH_FANCY_ICON * iUnitCount), (self.Y_STATS_TEXT + 30 + self.H_CIV_PANEL + 104) + (IconSize * yUnitMod), IconSize, IconSize, WidgetTypes.WIDGET_GENERAL, -1, -1 )
			iUnit += 1
			iUnitCount += 1
		...
		bodyString = localText.getText("TXT_KEY_ROYAL_CHARTER_BODY_2", (CyGameTextMgr().getTimeStr(gc.getGame().getGameTurn(), false), self.player.getCivilizationAdjectiveKey()))
		screen.addMultilineText( "BodyText", bodyString, self.X_TEXT_PANEL + self.iMarginSpace, self.Y_TEXT_PANEL + self.iMarginSpace + self.iTEXT_PANEL_MARGIN, self.W_TEXT_PANEL - (self.iMarginSpace * 2), self.H_TEXT_PANEL - (self.iMarginSpace * 2), WidgetTypes.WIDGET_GENERAL, -1, -1, CvUtil.FONT_LEFT_JUSTIFY)
