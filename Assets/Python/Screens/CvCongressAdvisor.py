	...
	def drawContents(self):
		...
		for iIndex in range(self.MaxColumnCount * 2):
			...
			if (iFatherIndex < len(self.MyFathers)):
				...
				if (bTechStart == False and iFatherCount == iTechCount - 1):
					bTechStart = True
					#continue
				...
				if (bTechStart == False):
					...
					screen.addDDSGFCAt("Portrait" + str(iIndex), "Background", gc.getCivicInfo(iFather).getCivicPortrait(), (iRecordWidth * iColumn) + (PortraitBorder * (iColumn + 1)) + ((iRecordWidth / 2) - (PortraitSize / 2)), (iRecordHight * iRow) + 30, PortraitSize - 50, PortraitSize - 50, WidgetTypes.WIDGET_GENERAL, -1, -1, False)
					szFatherTitle = gc.getCivicInfo(iFather).getDescription()
				...
				if (bTechStart == False):
					...
					szFatherData = u"<font=2>" + CyGameTextMgr().parseCivicInfo(iFather, False, False, True, False, self.player.getCivilizationType())[1:] + u"</font>"
					#screen.setLabel("TestText2", "Background", "<font=4>" + "Test 2" + "</font>", CvUtil.FONT_CENTER_JUSTIFY, (self.XResolution / 2) - 100, self.YResolution - 30, 0, FontTypes.TITLE_FONT, WidgetTypes.WIDGET_GENERAL, -1, -1)
