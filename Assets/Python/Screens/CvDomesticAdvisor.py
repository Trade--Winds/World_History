...
class CvDomesticAdvisor:
	...
	def interfaceScreen(self):
		...
		#TKs Med
		self.nNormalizedTableWidth = self.nTableWidth + 650
		
		
		self.iButtonSpacing = 80
		self.iButtonSize = 60
		self.iCityButtonSize = 34
		self.Y_LOWER_ROW = self.nScreenHeight - 70
		self.CITY_NAME_COLUMN_WIDTH = 140
		...
		self.PRODUCTION_COLUMN_SIZE = (self.nTableWidth - self.CITY_NAME_COLUMN_WIDTH) / 16
		self.WAREHOUSE_COLUMN_SIZE = (self.nTableWidth - self.CITY_NAME_COLUMN_WIDTH - 60) / 15
		self.BUILDING_COLUMN_SIZE = (self.nTableWidth - self.CITY_NAME_COLUMN_WIDTH) / 16
		self.ROW_HIGHT = 48
		#Tke
		self.Y_EXIT = self.nScreenHeight - 36
		self.X_EXIT = self.nScreenWidth - 30
		# TK INVENTOR MOD
		self.XResolution = self.nScreenWidth
		self.YResolution = self.nScreenHeight
		self.ICON_BUTTON_SIZE = 45
		self.AMENDMENT_BUTTON = 850
		# TK INVENTOR MOD
		...
		# TK INVENTOR MOD
		self.INVENTIONS_STATE = 7
		self.StateTypes.append("InventionState")
		self.StateButtons.append("INTERFACE_INVENTIONS_COMPLETE_SELECT")
		# TK INVENTOR MOD
		self.RebuildArrays()

		#Initialize the Lists
		for iState in range(len(self.StateTypes)):
			if iState != self.TRADEROUTE_STATE and iState != self.INVENTIONS_STATE:
				szStateName = self.StateTypes[iState] + "ListBackground"
				screen.addTableControlGFC(szStateName, 29, (self.nScreenWidth - self.nTableWidth - 60) / 2, 60, self.nTableWidth + 60, self.nTableHeight, True, False, self.iCityButtonSize, self.iCityButtonSize, TableStyles.TABLE_STYLE_STANDARD )
		...
		# Population Column
		screen.setTableColumnHeader( szListName, 2, "<font=3>" + localText.getText("TXT_KEY_POPULATION", ()) + "</font>", (self.nTableWidth - self.CITY_NAME_COLUMN_WIDTH) / iColumnMod )
		# Liberty Column
		screen.setTableColumnHeader( szListName, 3, "<font=3>" + (u" %c" %(CyGame().getSymbolID(FontSymbols.POWER_CHAR))) + "</font>", (self.nTableWidth - self.CITY_NAME_COLUMN_WIDTH) / iColumnMod )		
		# Food Column
		screen.setTableColumnHeader( szListName, 5, "<font=3>" + (u" %c" % gc.getYieldInfo(YieldTypes.YIELD_FOOD).getChar()) + "</font>", (self.nTableWidth - self.CITY_NAME_COLUMN_WIDTH) / iColumnMod )
		# Hammers Column
		screen.setTableColumnHeader( szListName, 6, "<font=3>" + (u" %c" % gc.getYieldInfo(YieldTypes.YIELD_HAMMERS).getChar()) + "</font>", (self.nTableWidth - self.CITY_NAME_COLUMN_WIDTH) / iColumnMod )
		# Bells Column
		screen.setTableColumnHeader( szListName, 8, "<font=3>" + (u" %c" % gc.getYieldInfo(YieldTypes.YIELD_BELLS).getChar()) + "</font>", (self.nTableWidth - self.CITY_NAME_COLUMN_WIDTH) / iColumnMod )
		# Crosses Column
		screen.setTableColumnHeader( szListName, 10, "<font=3>" + (u" %c" % gc.getYieldInfo(YieldTypes.YIELD_CROSSES).getChar()) + "</font>", (self.nTableWidth - self.CITY_NAME_COLUMN_WIDTH) / iColumnMod )
		# Education Column
		screen.setTableColumnHeader( szListName, 11, "<font=3>" + (u" %c" % gc.getYieldInfo(YieldTypes.YIELD_EDUCATION).getChar()) + "</font>", (self.nTableWidth - self.CITY_NAME_COLUMN_WIDTH) / iColumnMod )
		
		# Ideas Column
		screen.setTableColumnHeader( szListName, 12, "<font=3>" + (u" %c" % gc.getYieldInfo(YieldTypes.YIELD_IDEAS).getChar()) + "</font>", (self.nTableWidth - self.CITY_NAME_COLUMN_WIDTH) / iColumnMod )
		# Culture and Prosperity Column
		screen.setTableColumnHeader( szListName, 13, "<font=3>" + (u" %c" % gc.getYieldInfo(YieldTypes.YIELD_CULTURE).getChar()) + "</font>", (self.nTableWidth - self.CITY_NAME_COLUMN_WIDTH) / iColumnMod )
		# Garrison Column
		screen.setTableColumnHeader( szListName, 14, "<font=3>" + (u" %c" % CyGame().getSymbolID(FontSymbols.STRENGTH_CHAR)) + "</font>", (self.nTableWidth - self.CITY_NAME_COLUMN_WIDTH) / iColumnMod )
		# Defense Column
		screen.setTableColumnHeader( szListName, 15, "<font=3>" + (u" %c" % CyGame().getSymbolID(FontSymbols.DEFENSE_CHAR)) + "</font>", (self.nTableWidth - self.CITY_NAME_COLUMN_WIDTH) / iColumnMod )
		# Production Column
		screen.setTableColumnHeader( szListName, 16, "<font=3>" + localText.getText("TXT_KEY_DOMESTIC_ADVISOR_PRODUCING", ()).upper() + "</font>", (self.nTableWidth - self.CITY_NAME_COLUMN_WIDTH) / 4 )
		#Tke Med

		#WareHouseState Headers
		#screen.setTableColumnHeader( "WareHouseStateListBackground", 2, "<font=3>" + "MAX" + "</font>", 60 )
		for iYield in range(YieldTypes.YIELD_FOOD, YieldTypes.YIELD_PLATE_ARMOR + 1):
			screen.setTableColumnHeader( "WareHouseStateListBackground", iYield + 2, "<font=3> " + (u" %c" % gc.getYieldInfo(iYield).getChar()) + "</font>", ((self.WAREHOUSE_COLUMN_SIZE * self.nTableWidth) / self.nNormalizedTableWidth) - 1 )
	...
	def drawContents (self):
		...
		# TK INVENTOR MOD
		if (self.CurrentState != self.TRADEROUTE_STATE and self.CurrentState != self.INVENTIONS_STATE):
		...
		# TK INVENTOR MOD
		elif (self.CurrentState == self.INVENTIONS_STATE):
			#screen.hideScreen()
			self.drawCivics()
		# TK INVENTOR MOD
		else:
	...
	def drawCivics(self):
		...
		if(self.CurrentState == self.INVENTIONS_STATE):
			szIventionsName = self.StateTypes[self.INVENTIONS_STATE]
			player = gc.getPlayer(gc.getGame().getActivePlayer())
			BonusRow = 0
			Return = 0
			for iCivic in range(gc.getNumCivicInfos()):
				if (gc.getCivicInfo(iCivic).getCivicOptionType() == 5):
					if player.getIdeasResearched(iCivic):
						BonusRow += 1
						...
						screen.addDDSGFC(szIventionsName + str(iCivic), gc.getCivicInfo(iCivic).getButton(), (self.XResolution / 4) - (self.ICON_BUTTON_SIZE * gc.getNumCivicOptionInfos() ) + (self.ICON_BUTTON_SIZE * BonusRow * 2) + (self.ICON_BUTTON_SIZE / 2), 75 + Return, self.ICON_BUTTON_SIZE, self.ICON_BUTTON_SIZE, WidgetTypes.WIDGET_GENERAL, 500, iCivic)
						#if BonusRow == 2:
							#screen.hide(szIventionsName + str(iCivic))
						if BonusRow >= 12:
							Return = (self.ICON_BUTTON_SIZE / 3) + self.ICON_BUTTON_SIZE + Return
							BonusRow = 0
	...
	def updateCityTable(self, pLoopCity, i):
		...
		if(self.CurrentState == self.GENERAL_STATE):
			...
			# Culture rate
			screen.setTableInt(szState + "ListBackground", 13, i, "<font=3>" + unicode(pLoopCity.calculateNetYield(YieldTypes.YIELD_CULTURE)) + "</font>", "", WidgetTypes.WIDGET_GENERAL, -1, -1, CvUtil.FONT_LEFT_JUSTIFY )
			# Garrison
			screen.setTableInt(szState + "ListBackground", 14, i, "<font=3>" + unicode(pLoopCity.plot().getNumDefenders(pLoopCity.getOwner())) + "</font>", "", WidgetTypes.WIDGET_GENERAL, -1, -1, CvUtil.FONT_LEFT_JUSTIFY )
			# Defense
			szBuffer = u"<font=3>%s%%</font>" % (str(pLoopCity.getDefenseModifier()))
			screen.setTableInt(szState + "ListBackground", 15, i, "<font=3>" + szBuffer + "</font>", "", WidgetTypes.WIDGET_GENERAL, -1, -1, CvUtil.FONT_LEFT_JUSTIFY )
			# Producing
			screen.setTableText(szState + "ListBackground", 16, i, "<font=3>" + pLoopCity.getProductionName() + " (" + str(pLoopCity.getGeneralProductionTurnsLeft()) + ")" + "</font>", "", WidgetTypes.WIDGET_GENERAL, -1, -1, CvUtil.FONT_LEFT_JUSTIFY )
			#TKe
			...
			#TKs Med
			#screen.setTableInt("WareHouseStateListBackground", 2, i, u"<font=3><color=255,255,255>" + str(pLoopCity.getMaxYieldCapacity(-1)) + u"</color></font>", "", WidgetTypes.WIDGET_GENERAL, -1, -1, CvUtil.FONT_LEFT_JUSTIFY )
			...
			for iYield in range(YieldTypes.YIELD_FOOD, YieldTypes.YIELD_PLATE_ARMOR + 1):
				...
				if iNetYield == 0:
					szText = ""
				if (pLoopCity.calculateNetYield(iYield) * 5 + pLoopCity.getYieldStored(iYield) <= pLoopCity.getMaxYieldCapacity(iYield) or iYield == YieldTypes.YIELD_FOOD):
					screen.setTableInt("WareHouseStateListBackground", iYield + 2, i, u"<font=3><color=0,255,255>" + szText + u"</color></font>", "", WidgetTypes.WIDGET_GENERAL, -1, -1, CvUtil.FONT_LEFT_JUSTIFY )
				elif (pLoopCity.getYieldStored(iYield) <= pLoopCity.getMaxYieldCapacity(iYield)):
					#Tke
					screen.setTableInt("WareHouseStateListBackground", iYield + 2, i, u"<font=3><color=255,255,0>" + szText + u"</color></font>", "", WidgetTypes.WIDGET_GENERAL, -1, -1, CvUtil.FONT_LEFT_JUSTIFY )
				else:
					screen.setTableInt("WareHouseStateListBackground", iYield + 2, i, u"<font=3><color=255,0,0>" + szText + u"</color></font>", "", WidgetTypes.WIDGET_GENERAL, -1, -1, CvUtil.FONT_LEFT_JUSTIFY )
	...
	def updateCitizenTable(self, pCity, iRow):
		...
		if(self.CurrentState == self.CITIZEN_STATE):
			...
			ButtonSizePercentage = 60
			UnitButtonSizePercentage = self.iCityButtonSize + 5
			for iCitizen in range(pCity.getPopulation() - 1, -1, -1):
				...
				screen.setImageButtonAt(szButtonName, "CitizenPanel" + str(iRow), gc.getUnitInfo(iType).getButton(), iCitizen * iSpace, UnitButtonSizePercentage / 5, UnitButtonSizePercentage * 4 / 5, UnitButtonSizePercentage * 4 / 5, WidgetTypes.WIDGET_CITIZEN, pCitizen.getID(), pCity.getID())
	...
	def RebuildTransportTable (self):
		...
		screen.addTableControlGFC( szStateName + "ListBackground", 29, (self.nScreenWidth - self.nTableWidth) / 2, 60, self.nTableWidth, self.nTableHeight, True, False, self.iCityButtonSize, self.iCityButtonSize, TableStyles.TABLE_STYLE_STANDARD )
	...
	def RebuildRouteTable (self):
		...
		screen.addTableControlGFC( szTableName, 29, (self.nScreenWidth - self.nTableWidth) / 2, 60, self.nTableWidth, self.nTableHeight, True, False, self.iCityButtonSize, self.iCityButtonSize, TableStyles.TABLE_STYLE_STANDARD )
						...
						if (self.CurrentState == self.INVENTIONS_STATE):
							...
							for iCivic in range(gc.getNumCivicInfos()):
								if (gc.getCivicInfo(iCivic).getCivicOptionType() == 5):
									if player.getIdeasResearched(iCivic):
										#BonusRow += 1
										screen.hide(self.StateTypes[self.CurrentState] + str(iCivic))
						#TK INVENTIONS
	...
	def getWidgetHelp(self, argsList):
		...
		if eWidgetType == WidgetTypes.WIDGET_GENERAL:
			...
			# TK INVENTOR MOD
			elif iData1 == self.INVENTIONS_STATE:
				return localText.getText("TXT_KEY_TECHONOLOGY_ADVISOR_COMPLETED", ())
			elif iData1 == 10001:
				unit = gc.getActivePlayer().getUnit(iData2)
				if not unit.isNone():
					return CyGameTextMgr().getSpecificUnitHelp(unit, true, false)
			elif iData1 == 500:
				szHelp = CyGameTextMgr().parseCivicInfo(iData2, False, False, True, False)
