...
#MultipleYieldsProduced End	
		...
		for iCivic in range(gc.getNumCivicInfos()):
			iAllows = gc.getCivicInfo(iCivic).getAllowsYields(self.iYields)
			if (iAllows > 0):
				szFatherTitle = gc.getCivicInfo(iCivic).getDescription()
				szTextTech = localText.getText("TXT_KEY_TECH_MAKES_AVAILABLE", (szFatherTitle,))
				#szTextTech = "HOwdy Yo"
				screen.appendListBoxStringNoUpdate(panelName, u"<font=4>" + szTextTech + u"</font>", WidgetTypes.WIDGET_GENERAL, 0, 0, CvUtil.FONT_LEFT_JUSTIFY)
				break
			elif (iAllows < 0):
				szFatherTitle = gc.getCivicInfo(iCivic).getDescription()
				szTextTech = localText.getText("TXT_KEY_TECH_MAKES_OBSOLETE", (szFatherTitle,))
				screen.appendListBoxStringNoUpdate(panelName, u"<font=4>" + szTextTech + u"</font>", WidgetTypes.WIDGET_GENERAL, 0, 0, CvUtil.FONT_LEFT_JUSTIFY)
				break
		#TKe
