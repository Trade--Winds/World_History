...
class CvDiplomacy:
	...
	def determineResponses (self, eComment):
		...
		# If this is the first time we are being contacted by the AI
			...
			#Tks Med Diplo
			self.addUserComment("USER_DIPLOCOMMENT_INSULT_FIRSTCONTACT", -1, -1)
		...
		#Tks Med
		elif (self.isComment(eComment, "AI_DIPLOCOMMENT_OFFER_VASSAL")):
			# We can accept their offer
			self.addUserComment("USER_DIPLOCOMMENT_ACCEPT_VASSAL", -1, -1)
			# Demand your lands
			self.addUserComment("USER_DIPLOCOMMENT_DEMAND_LAND", -1, -1)
			# Or reject it...
			self.addUserComment("USER_DIPLOCOMMENT_REJECT_OFFER", -1, -1)
			
		# If the AI is offering a city or
		...
		elif (self.isComment(eComment, "AI_DIPLOCOMMENT_KING_ASK_FOR_GOLD_OR_ELSE")):
			# We can accept their demands
			self.addUserComment("USER_DIPLOCOMMENT_KING_GIVE_GOLD", -1, -1)
			# Or reject them...
			self.addUserComment("USER_DIPLOCOMMENT_KING_REFUSE_GOLD_OR_ELSE", -1, -1)
		...
		#TKs Med
		elif (self.isComment(eComment, "AI_DIPLOCOMMENT_KING_REFUSE_GOLD_OR_ELSE")):

			self.addUserComment("USER_DIPLOCOMMENT_KING_REFUSED_GOLD_OR_ELSE", -1, -1)
		...
		elif (self.isComment(eComment, "AI_DIPLOCOMMENT_MEET_THE_POP")):

			self.addUserComment("USER_DIPLOCOMMENT_MEET_THE_POP", -1, -1)
			#Tks Med
			self.addUserComment("USER_DIPLOCOMMENT_MEET_THE_POP_INSULT", -1, -3)
			
		elif (self.isComment(eComment, "AI_DIPLOCOMMENT_FOUND_FIRST_CITY")):

			self.addUserComment("USER_DIPLOCOMMENT_FOUND_FIRST_CITY", -1, -1)
			
			self.addUserComment("USER_DIPLOCOMMENT_FOUND_FIRST_CITY_INSULT", -1, -2)
			#Tke
		...
		# If they reject our demand
				...
				# Otherwise this is a player offer to the AI
					...
					# Otherwise they have something on the table and we dont
						...
						# Otherwise (during peacetime) ask what they want for our item or demand they give it to us
							...
							if (gc.getGame().getActiveTeam() == gc.getPlayer(self.diploScreen.getWhoTradingWith()).getTeam()):
								if (gc.getPlayer(self.diploScreen.getWhoTradingWith()).canMakeVassalDemand(gc.getGame().getActivePlayer())):
			...
			if (gc.getTeam(gc.getGame().getActiveTeam()).canDeclareWar(gc.getPlayer(self.diploScreen.getWhoTradingWith()).getTeam())):
				if gc.getTeam(gc.getPlayer(self.diploScreen.getWhoTradingWith()).getTeam()).isParentOf(gc.getGame().getActiveTeam()):
					...
					self.addUserComment("USER_DIPLOCOMMENT_WAR", -1, -1)
				#TKs Med
				self.addUserComment("USER_DIPLOCOMMENT_SOMETHINGELSE_INSULT", -1, -1)
				#TKe
			#if (gc.getTeam(gc.getGame().getActiveTeam()).canDeclareWar(gc.getPlayer(self.diploScreen.getWhoTradingWith()).getTeam())):
	...
	def performHeadAction( self, eComment ):
		...
		if ( eComment == self.getCommentID("AI_DIPLOCOMMENT_NO_PEACE") or
			...
			self.diploScreen.performHeadAction( LeaderheadAction.LEADERANIM_GREETING )
		       ...
		       #Tks Med
		       eComment == self.getCommentID("AI_DIPLOCOMMENT_STOP_DENIED") or
		       eComment == self.getCommentID("AI_DIPLOCOMMENT_INSULT_RESPONSE")):
			#Tke
		...
		#TKs Med
		       ...
		       eComment == self.getCommentID("AI_DIPLOCOMMENT_KING_ASK_FOR_GOLD_OR_ELSE") or
			...
			CyInterface().playGeneralSound("AS2D_KISS_MY_RING")
		      ...
		      eComment == self.getCommentID("AI_DIPLOCOMMENT_KING_REFUSE_GOLD_OR_ELSE") or
	...
	def handleUserResponse(self, eComment, iData1, iData2):
		...
		#TKs Med
		elif (self.isComment(eComment, "USER_DIPLOCOMMENT_INSULT_FIRSTCONTACT")):
			diploScreen.diploEvent(DiploEventTypes.DIPLOEVENT_INSULT_FIRSTCONTACT, diploScreen.getData(), -1)
			diploScreen.closeScreen()
		elif (self.isComment(eComment, "USER_DIPLOCOMMENT_SOMETHINGELSE_INSULT")):
			diploScreen.diploEvent(DiploEventTypes.DIPLOEVENT_INSULT_FIRSTCONTACT, diploScreen.getData(), -1)
			self.setAIComment(self.getCommentID("AI_DIPLOCOMMENT_INSULT_RESPONSE"))
		elif (self.isComment(eComment, "USER_DIPLOCOMMENT_FOUND_FIRST_CITY_INSULT")):
			diploScreen.diploEvent(DiploEventTypes.DIPLOEVENT_FOUND_CITY, iData1, iData2)
			diploScreen.closeScreen()
		elif (self.isComment(eComment, "USER_DIPLOCOMMENT_MEET_THE_POP_INSULT")):
			diploScreen.diploEvent(DiploEventTypes.DIPLOEVENT_FOUND_CITY, iData1, iData2)
			diploScreen.closeScreen()
		elif (self.isComment(eComment, "USER_DIPLOCOMMENT_DEMAND_LAND")):
			diploScreen.diploEvent(DiploEventTypes.DIPLOEVENT_DEMAND_VASSAL_LANDS, diploScreen.getData(), -1)
			#self.setAIComment(self.getCommentID("AI_DIPLO_GIVE_VASSAL_LANDS"))
			diploScreen.closeScreen()
		elif (self.isComment(eComment, "USER_DIPLOCOMMENT_ACCEPT_VASSAL")):
			diploScreen.diploEvent(DiploEventTypes.DIPLOEVENT_ACCEPT_VASSAL, diploScreen.getData(), -1)
			diploScreen.closeScreen()
		#Tke
		# If we choose war
		...
		# if we are demanding something from our teammate
			...
			iDecay = gc.getLeaderHeadInfo(gc.getPlayer(self.diploScreen.getWhoTradingWith()).getPersonalityType()).getMemoryDecayRand(MemoryTypes.MEMORY_MADE_VASSAL_DEMAND)
			iVassalDelay = CyGame().getSorenRandNum(iDecay, "Vassal Demand Delay")
			iVassalDelay = 1 + iVassalDelay
			gc.getPlayer(self.diploScreen.getWhoTradingWith()).AI_changeMemoryCount(gc.getGame().getActivePlayer(), MemoryTypes.MEMORY_MADE_VASSAL_DEMAND, iVassalDelay)
			#gc.getPlayer(self.diploScreen.getWhoTradingWith()).AI_changeMemoryCount(gc.getGame().getActivePlayer(), MemoryTypes.MEMORY_INSULTED, 1)
		...
		elif (self.isComment(eComment, "USER_DIPLOCOMMENT_KING_REFUSE_GOLD_OR_ELSE")):
			self.setAIComment(self.getCommentID("AI_DIPLOCOMMENT_KING_REFUSE_GOLD_OR_ELSE"))
		...
		elif (self.isComment(eComment, "USER_DIPLOCOMMENT_KING_REFUSED_GOLD_OR_ELSE")):
			diploScreen.diploEvent(DiploEventTypes.DIPLOEVENT_REFUSE_TAX_RATE, diploScreen.getData(), -1)
			diploScreen.closeScreen()
		...
		elif (self.isComment(eComment, "USER_DIPLOCOMMENT_MEET_THE_POP")):
			diploScreen.diploEvent(DiploEventTypes.DIPLOEVENT_FOUND_CITY, diploScreen.getData(), false)
			diploScreen.closeScreen()
