...
void CvSelectionGroup::doTurn()
{
	...
	if (getNumUnits() > 0)
	{
		...
		///TKs Med
        if (eActivityType == ACTIVITY_COLLECT_TAXES)
        {
            if (!pLoopUnit->collectTaxes())
            {
                setActivityType(ACTIVITY_AWAKE);
            }
        }
...
bool CvSelectionGroup::canStartMission(int iMission, int iData1, int iData2, CvPlot* pPlot, bool bTestVisible, bool bUseCache)
{
	...
	while (pUnitNode != NULL)
	{
        ...
        ///TKs Med
		case MISSION_FOUND:
			if (pLoopUnit->canFound(pPlot, bTestVisible, GC.getDefineINT("FOUND_VILLAGE_NUMBER")))
			{
                if (pLoopUnit->getProfession() != NO_PROFESSION)
                {
                    MedCityTypes eCityType = (MedCityTypes)GC.getProfessionInfo(pLoopUnit->getProfession()).getFoundCityType();
		...
		case MISSION_FOUND_MONASTERY:
            if (pLoopUnit->canFound(pPlot, bTestVisible, GC.getDefineINT("FOUND_MONASTERY_NUMBER")))
			{
			   if (pLoopUnit->getProfession() != NO_PROFESSION)
			   {
                   if ((MedCityTypes)GC.getProfessionInfo(pLoopUnit->getProfession()).getFoundCityType() == CITYTYPE_MONASTERY)
                   {
                        return true;
                   }
			   }
			...
			break;
        case MISSION_FOUND_OUTPOST:
            if (pLoopUnit->canFound(pPlot, bTestVisible, GC.getDefineINT("FOUND_OUTPOST_NUMBER")))
			{
			  if (pLoopUnit->getProfession() != NO_PROFESSION)
			  {
                  if ((MedCityTypes)GC.getProfessionInfo(pLoopUnit->getProfession()).getFoundCityType() == CITYTYPE_OUTPOST)
                  {
                    return true;
                  }
			  }
		...
		case MISSION_COLLECT_TAXES:
            if (pLoopUnit->canCollectTaxes(bTestVisible))
			{
				return true;
			}
			break;
...
void CvSelectionGroup::startMission()
{
	...
	else
	{
        ...
        ///TKs Med
        case MISSION_COLLECT_TAXES:
            setActivityType(ACTIVITY_COLLECT_TAXES);
			bNotify = true;
			bDelete = true;
			break;
		case MISSION_BOMBARD:
			...
			if (canAllMove())
			{
				...
				while (pUnitNode != NULL && !bBreak)
				{
					...
					if (pLoopUnit->canMove())
					{
						...
						{
						case MISSION_FOUND:
							if (pLoopUnit->found(GC.getDefineINT("FOUND_VILLAGE_NUMBER")))
							{
								bAction = true;
								bBreak = true;
							}
                        ...
                        ///TKs Med
                        case MISSION_FOUND_MONASTERY:
                            if (pLoopUnit->found(GC.getDefineINT("FOUND_MONASTERY_NUMBER")))
							{
								bAction = true;
								bBreak = true;
							}
							...
							break;
                        case MISSION_FOUND_OUTPOST:
                            if (pLoopUnit->found(GC.getDefineINT("FOUND_OUTPOST_NUMBER")))
							{
								bAction = true;
								bBreak = true;
							}
...
bool CvSelectionGroup::groupAttack(int iX, int iY, int iFlags, bool& bFailedAlreadyFighting)
{
	...
	if (getNumUnits() > 0)
	{
		...
		{
			if ((iFlags & MOVE_DIRECT_ATTACK) || (generatePath(plot(), pDestPlot, iFlags) && (getPathFirstPlot() == pDestPlot)))
			{
				int iAttackOdds;
