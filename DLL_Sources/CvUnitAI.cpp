...
///Tke
	...
	if (!AI_afterAttack())
	{
		if (getGroup()->isAutomated() && (getGroup()->getAutomateType() != AUTOMATE_FULL))
		{
            ...
            ///TKs Med
			case AUTOMATE_SAIL:
				AI_sailToEurope(false, NO_EUROPE);
				break;
            ///TKe
            ...
            case AUTOMATE_TRAVEL_SILK_ROAD:
                AI_sailToEurope(true, NO_EUROPE);
                break;
...
int CvUnitAI::AI_attackOdds(const CvPlot* pPlot, bool bPotentialEnemy) const
{
	...
	iDamageToUs = std::max(1,((GC.getXMLval(XML_COMBAT_DAMAGE) * (iTheirFirepower + iStrengthFactor)) / (iOurFirepower + iStrengthFactor)));
	iDamageToThem = std::max(1,((GC.getXMLval(XML_COMBAT_DAMAGE) * (iOurFirepower + iStrengthFactor)) / (iTheirFirepower + iStrengthFactor)));
    ...
    for (iI = 0; iI < NUM_DIRECTION_TYPES; iI++)
	{
		...
		if (pLoopPlot != NULL)
		{
		    if (AI_plotValid(pLoopPlot) && pLoopPlot->isVisible(getTeam(), false))
            {
                ...
                {
                    if (!atPlot(pLoopPlot) && canMoveInto(pLoopPlot, true))
                    {
			...
			if (AI_plotValid(pAdjacentPlot))
			{
				...
				if (!pAdjacentPlot->isCity() && !pAdjacentPlot->isEurope() && pAdjacentPlot->getNumUnits() == 0)
				{
					if (generatePath(pAdjacentPlot, 0, true))
					{
                        ...
                        iRand = getUnitInfo().getAnimalPatrolWeight();
                        // Move this Global Define Int to a UnitInfo tag - iAnimalPatrolWeight
                        //iRand = GC.getDefineINT("WILD_ANIMAL_SEA_PATROL_BASE_WEIGHT");
                        if (iRand > 0)
                        {
...
void CvUnitAI::AI_treasureMove()
{
	...
	if (pCity != NULL)
	{
		...
		else if (!GET_PLAYER(getOwnerINLINE()).AI_hasSeaTransport(this) || bAtWar)
		{
			if (canKingTransport())
			{
				kingTransport(true);
	...
	if (pCity != NULL)
	{
		...
		if (pCity == getHomeCity())
		{
			if (pCity->AI_isDefended(-1))
			{
				...
				{
					bool bMakeTribute = AI_getTributeColony(ATTITUDE_CAUTIOUS, ATTITUDE_FRIENDLY, 5);

					if (bMakeTribute == true)
					{
						//FAssertMsg(bMakeTribute == false, "This dumb ass should be false");
						iGiftTimer = std::max(0, iGiftTimer - GC.getXMLval(XML_CONTACT_YIELD_GIFT_ENCOMEIDA_TIMER));
...
///TKs Invention Core Mod v 1.0
bool CvUnitAI::AI_getTributeColony(int iMinAttitude, int iMaxAttitude, int iRange)
{
	//Test
	//return false;
	int iX, iY;
	...
	for (iX = -iRange; iX <= iRange; iX++)
	{
		for (iY = -iRange; iY <= iRange; iY++)
		{
			...
			if (NULL != pLoopPlot)
			{
				if (pLoopPlot->isCity() && pLoopPlot->getArea() == getArea())
				{
					CvPlayer& kLoopPlayer = GET_PLAYER(pLoopPlot->getOwner());
                        ...
                        {
                            //CvCity* eVassal = pLoopPlot->getPlotCity();
                            if (kLoopPlayer.getID() == GET_PLAYER(getOwnerINLINE()).getVassalOwner())
                            {
                                int iPathTurns;
                            ...
                            ///TK Update Stripped
//                            if ((CivicTypes)GC.getDefineINT("CONTACT_YIELD_GIFT_TECH") != NO_CIVIC)
//                            {
//                                if (kLoopPlayer.getIdeasResearched((CivicTypes)GC.getDefineINT("CONTACT_YIELD_GIFT_TECH")) > 0)
//                                {
//                                        int iPathTurns;
//
//                                        if (generatePath(pLoopPlot, 0, true, &iPathTurns))
...
bool CvUnitAI::AI_europeBuyNativeYields()
{
	...
	yields.push_back(YIELD_WEAPONS);
	...
	if (kOwner.isYieldEuropeTradable(YIELD_SCALE_ARMOR))
	{
        ...
        {
            yields.push_back(YIELD_SCALE_ARMOR);
        }
	}

	if (kOwner.isYieldEuropeTradable(YIELD_MAIL_ARMOR))
	{
        if (GC.getGameINLINE().getSorenRandNum(2, "AI buy mail armor") == 0)
        {
            yields.push_back(YIELD_MAIL_ARMOR);
        }
	}

	if (kOwner.isYieldEuropeTradable(YIELD_PLATE_ARMOR))
	{
        if (GC.getGameINLINE().getSorenRandNum(4, "AI buy plate armor") == 0)
        {
...
///TKs Med
bool CvUnitAI::AI_sailToEurope(bool bMove, EuropeTypes eTradeScreenType)
{
	...
	if (isHuman())
	{
		...
		{
			eTradeScreenType = getUnitTradeMarket();
		}
	}
	...
	if (canCrossOcean(plot(), eTravelState, NO_TRADE_ROUTES, false, eTradeScreenType))
	{
		
		crossOcean(UNIT_TRAVEL_STATE_TO_EUROPE, false, eTradeScreenType);
		AI_setUnitAIState(UNITAI_STATE_DEFAULT);
		...
		setUnitTradeMarket(eTradeScreenType);
		return true;
	}
	...
	for (int iI = 0; iI < GC.getMapINLINE().numPlotsINLINE(); iI++)
	{
		...
		if (AI_plotValid(pLoopPlot) && !pLoopPlot->isVisibleEnemyDefender(this))
		{
			...
			{
				if (canCrossOcean(pLoopPlot, eTravelState, NO_TRADE_ROUTES, false, eTradeScreenType))
				{
				    if (bLandRoute)
				    {
	...
	else if (!isHuman() && pBestPlot == NULL && bLandRoute)
    {
        for (int iI = 0; iI < GC.getMapINLINE().numPlotsINLINE(); iI++)
        {
            ...
            if (AI_plotValid(pLoopPlot) && !pLoopPlot->isVisibleEnemyDefender(this))
            {
                if (canCrossOcean(pLoopPlot, eTravelState, NO_TRADE_ROUTES, true))
                {
                    int iPathTurns;
                    if (generatePath(pLoopPlot, MOVE_BUST_FOG, true, &iPathTurns))
                    {
	...
	if (pBestPlot != NULL)
	{
		...
		if (plot()->isTradeScreenAccessPlot(eTradeScreenType))
		{
			if (canCrossOcean(plot(), eTravelState, NO_TRADE_ROUTES, false, eTradeScreenType))
			{
				crossOcean(UNIT_TRAVEL_STATE_TO_EUROPE, false, eTradeScreenType);
...
///TKs Med
void CvUnitAI::AI_doFound(int iType)
{
	doFound(false, iType);
}
///TKe
...
ProfessionTypes CvUnitAI::AI_getIdealProfession() const
{
	...
	///TKs MEd
	FAssert(m_eIdealProfessionCache != INVALID_PROFESSION);
	///TKe
...
bool CvUnitAI::AI_betterJob()
{
	...
	if (bJoined)
	{
		if (pCity->removePopulationUnit(this, false, eOriginalProfession))
		{
...
bool CvUnitAI::AI_found(int iMinValue)
{
	...
	bool bTransportPath = false;
    ///TKs Med
    bool bWaterStartNoCity = (!GC.getCivilizationInfo(getCivilizationType()).isWaterStart() && kOwner.getNumCities() == 0);

	for (int iI = 0; iI < GC.getMapINLINE().numPlotsINLINE(); iI++)
	{
		CvPlot* pLoopPlot = GC.getMapINLINE().plotByIndexINLINE(iI);

		if ((pLoopPlot->isRevealed(getTeam(), false) || bWaterStartNoCity) && ((pTransportUnit != NULL) || AI_plotValid(pLoopPlot)))
		{
		    ///TKe
		...
		if (!atPlot(pLoopPlot))
		{
			...
			if (pTransportUnit->generatePath(pLoopPlot, 0, true, &iPathTurns))
			{
				if (iPathTurns < iMaxPath)
				{
					...
					///Tks Med
					iValue += pCity->getMaxYieldCapacity(getYield()) - pCity->getYieldStored(getYield());
					///TKe
							...
							if (iIndex != CITY_HOME_PLOT)
							{
								if (((NULL == pIgnoreCity) || ((pCity->AI_getWorkersNeeded() > 0) && (pCity->AI_getWorkersHave() < (1 + pCity->AI_getWorkersNeeded() * 2 / 3)))) && (pCity->AI_getBestBuild(iIndex) != NO_BUILD))
								{
									if (canBuild(pLoopPlot, pCity->AI_getBestBuild(iIndex)))
									{
										...
										{
											if (pLoopPlot->getImprovementType() != NO_IMPROVEMENT && pLoopPlot->getImprovementType() != GC.getXMLval(XML_RUINS_IMPROVEMENT))
											{
												bAllowed = false;
											}
										}
		...
		for (CvCity* pLoopCity = GET_PLAYER(getOwnerINLINE()).firstCity(&iLoop); pLoopCity != NULL; pLoopCity = GET_PLAYER(getOwnerINLINE()).nextCity(&iLoop))
		{
			...
			if (AI_plotValid(pLoopPlot))
			{
				if (!(pLoopPlot->isVisibleEnemyUnit(this)))
				{
					...
					if (bWaterStart && pLoopPlot->getNearestEurope() == eMainEurope)
					{
						int iPathTurns = 0;
					...
					else if (!bWaterStart)
					{
					    ...
					    if (atPlot(pLoopPlot) || generatePath(pLoopPlot, ((iPass > 0) ? MOVE_IGNORE_DANGER : 0), true, &iPathTurns))
						{
							...
							{
								int iValue = 3;
								if (GC.getUnitInfo(getUnitType()).getConvertsToBuildingClass() != NO_BUILDINGCLASS)
                                {
                                    if (pLoopCity->getCityType() != CITYTYPE_MONASTERY)
                                    {
                                        iValue *= 100;
                                    }
                                }
...
int CvUnitAI::AI_finalOddsThreshold(CvPlot* pPlot, int iOddsThreshold)
{
	...
	if (pCity != NULL)
	{
		...
		{
			iFinalOddsThreshold += std::max(0, (pCity->getDefenseDamage() - pCity->getLastDefenseDamage() - (GC.getXMLval(XML_CITY_DEFENSE_DAMAGE_HEAL_RATE) * 2)));
		}
	}
...
bool CvUnitAI::AI_maraudTowardsVictimCity()
{
	int iBestValue = 0;
	CvPlot* pBestPlot = NULL;
	CvPlot* pBestMissionPlot = NULL;
		...
		if (pCity != NULL)
		{
			if (isEnemy(pCity->getTeam()))
			{
				if (!pCity->isNative())
				{
                    ...
                    int iCityValue = 1 + GC.getGameINLINE().getSorenRandNum(pCity->getPopulation() * 1000, "AI Maraud City Target");
                    for (int iDirection = 0; iDirection < NUM_DIRECTION_TYPES; iDirection++)
                    {
                        CvPlot* pDirectionPlot = plotDirection(pLoopPlot->getX_INLINE(), pLoopPlot->getY_INLINE(), (DirectionTypes)iDirection);
                        if (pDirectionPlot != NULL)
                        {
                            int iPathTurns;
                            if (generatePath(pDirectionPlot, MOVE_BUST_FOG, true, &iPathTurns))
                            {
                                int iValue = iCityValue / 10;
                                iValue += pLoopPlot->getCrumbs();
                                iValue /= 2 + std::max(3, iPathTurns);
                                if (iPathTurns < 3)
                                {
                                ...
                                CvPlayer& kCityOwner = GET_PLAYER(pCity->getOwnerINLINE());
                                if (GET_TEAM(kCityOwner.getTeam()).countNumCitiesByArea(area()) == 1)
                                {
                                    iValue *= 3;
                                    iValue /= 2;
                                }
...
bool CvUnitAI::AI_moveTowardsPilgramCity()
{
	int iBestValue = 0;
	CvPlot* pBestPlot = NULL;
	CvPlot* pBestMissionPlot = NULL;
	...
	for (int iI = 0; iI < GC.getMapINLINE().numPlotsINLINE(); iI++)
	{
		...
		if (pCity != NULL)
		{
			if (pCity->isHasBuildingClass((BuildingClassTypes)GC.getXMLval(XML_DEFAULT_SHRINE_CLASS)))
			{
...
//                                }
	...
	{
		FAssert(!atPlot(pBestPlot));
		getGroup()->pushMission(MISSION_MOVE_TO, pBestPlot->getX_INLINE(), pBestPlot->getY_INLINE(), 0, false, false, MISSIONAI_PILGRAM, pBestMissionPlot);
		return true;
	}
...
//	}
    ...
    if (AI_continueMission(0, MISSIONAI_PILGRAM, 0))
	{
		return;
	}

	if (AI_moveTowardsPilgramCity())
	{
		return;
	}
...
//		}
	...
	//if (bDanger)
	//{
		if (AI_huntAttack(6, 60, 100, NULL))
		{
			return;
		}
...
//    }
    ...
    if (bReturn == true)
    {
        bArrived = pUpgradeCity->plot() == pPlot && canMove();
        if (bArrived && eTradeRouteType == TRADE_ROUTE_FAIR)
        {
            getGroup()->setAutomateType(NO_AUTOMATE);
            crossOcean(UNIT_TRAVEL_STATE_TO_TRADE_FAIR);

            return true;
        }
        else if (bArrived && eTradeRouteType == TRADE_ROUTE_SILK_ROAD)
        {
            getGroup()->setAutomateType(NO_AUTOMATE);
...
bool CvUnitAI::AI_huntAttack(int iRange, int iLowOddsThreshold, int iHighOddsThreshold, CvPlot* pHintPlot)
{
	CvPlot* pLoopPlot;
	CvPlot* pBestPlot;
	CvCity* pCity;
	...
	for (iDX = -(iSearchRange); iDX <= iSearchRange; iDX++)
	{
		for (iDY = -(iSearchRange); iDY <= iSearchRange; iDY++)
		{
			...
			if (pLoopPlot != NULL)
			{
				if (AI_plotValid(pLoopPlot))
				{
					if (pLoopPlot->isVisibleBarbarianEnemyUnit(this))
					{
						if (!atPlot(pLoopPlot) && (generatePath(pLoopPlot, 0, true, &iPathTurns) && (iPathTurns <= iRange)))
						{
						    //FAssert();
							//if (pLoopPlot->getNumVisibleEnemyDefenders(this) > 0)
							{
								...
								if (iValue >= AI_finalOddsThreshold(pLoopPlot, iLowOddsThreshold))
								{
									if (iValue > iBestValue)
									{
										iBestValue = iValue;
...
//			if (pLoopPlot != NULL)
//			{
//				if (canMoveOrAttackInto(pLoopPlot, true) && !pLoopPlot->isCity())
//				{
//				    CLLNode<IDInfo>* pUnitNode;
//                    CvUnit* pLoopUnit;
//                    bool bFoundAnimal = false;
//                    pUnitNode = pLoopPlot->headUnitNode();
//                    while (pUnitNode != NULL)
//                    {
//                        pLoopUnit = ::getUnit(pUnitNode->m_data);
//                        pUnitNode = pLoopPlot->nextUnitNode(pUnitNode);
//
//                        if (GC.getGameINLINE().isBarbarianPlayer(pLoopUnit->getOwnerINLINE()))
//                        {
