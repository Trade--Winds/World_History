...
///TK MED TESTING
//bool bFoundHim = false;
//if (getCivilizationType() == (CivilizationTypes)GC.getDefineINT("DEFAULT_TEST_DEFINE"))
//{
//    bFoundHim = true;
//}
///TK TESTING END
...
int CvCityAI::AI_buildingValue(BuildingTypes eBuilding, int iFocusFlags) const
{
	...
	if (isNative())
	{
	    for (int i = 0; i < NUM_YIELD_TYPES; ++i)
		{
		    int iYieldCost = kBuildingInfo.getYieldCost(i);
			if ((iYieldCost > 0) && (i != YIELD_HAMMERS))
			{
				if ((iYieldCost - 5) > getYieldStored((YieldTypes)i))
				{
				    return 0;
				}
	...
	if (!bIsBadProfession)
	{
		...
		for (int i = 0; i < NUM_YIELD_TYPES; ++i)
		{
			...
			int iAdded = 0;
            ///TKs Med
            YieldTypes eMustSaleYield = (YieldTypes)kBuildingInfo.getAutoSellYieldChange();
            if (eMustSaleYield == NO_YIELD)
            {
                iAdded += kBuildingInfo.getYieldChange(eLoopYield);
            }
            else if (kBuildingInfo.getYieldChange(eLoopYield) > 0)
            {
                ...
                if (isMarket(eMustSaleYield) && getYieldStored(eMustSaleYield) >= (getMaintainLevel(eMustSaleYield) + 1))
                {
                    ...
                    else if ((kBuildingInfo.getYieldChange(eLoopYield) - iLoss) > 0)
                    {
                        iAdded += kBuildingInfo.getYieldChange(eLoopYield) - iLoss;
                    }
                }
	...
	if (GET_PLAYER(getOwnerINLINE()).AI_isStrategy(STRATEGY_REVOLUTION_PREPARING))
	{
		if (plot()->getNearestEurope() != NO_EUROPE)
		{
			...
			if (GET_PLAYER(getOwnerINLINE()).AI_isStrategy(STRATEGY_REVOLUTION_DECLARING))
			{
			    ...
			    if (GET_PLAYER(getOwner()).getParent() != NO_PLAYER)
			    {
			        int iCities = 0;
			        iCities = GET_PLAYER(getOwnerINLINE()).getNumCities();
			        if (iCities > 0)
			        {
                        CvPlayer& kOwner = GET_PLAYER(getOwnerINLINE());

                        int iKingsUnits =  GET_PLAYER(kOwner.getParent()).getNumRevolutionEuropeUnits() / iCities;
                        iDefenders += iKingsUnits;
			        }
...
void CvCityAI::AI_doNativeTrade()
{
	...
	for (int iI = 0; iI < NUM_YIELD_TYPES; iI++)
	{
		...
		///Tks Med
		if (GC.getYieldInfo(eYield).isCargo() && GC.getYieldInfo(eYield).getNativeSellPrice() == -1)
		{
			///Tke
			...
			if (iValue > 0)
			{
				...
				if (iValue > AI_getRequiredYieldLevel(eYield) || iProduced > 0)
				{
					...
					///TKs
					else if (GC.isEquipmentType(eYield, EQUIPMENT_ANY))
					{
						iValue *= 2;
					}
					///Tke
	...
	for (pLoopCity = kOwner.firstCity(&iLoop); pLoopCity != NULL; pLoopCity = kOwner.nextCity(&iLoop))
	{
		if (pLoopCity != this)
		{
			...
			if (GC.isEquipmentType(eBestYield, EQUIPMENT_ARMOR_HORSES))
			{
...
void CvCityAI::AI_doNative()
{
	...
	for (int iI = 0; iI < NUM_YIELD_TYPES; iI++)
	{
		YieldTypes eYield = (YieldTypes)iI;
		///TKs
		if ((eYield != YIELD_FOOD) && !GC.isEquipmentType(eYield, EQUIPMENT_ANY))
		{
		    ///Tke
			...
			if (iAmountLost > 0)
			{
				...
				{
				    ///TKs Med
					kPlayer.changeGold(((iAmountLost * iSellPrice) * GC.getDefineINT("NATIVE_AUTO_SELL_PERCENT")) / 100);
					///TKe
				}
			}
		}
...
int CvCityAI::AI_professionValue(ProfessionTypes eProfession, const CvUnit* pUnit, const CvPlot* pPlot, const CvUnit* pDisplaceUnit) const
{
    ...
    ///TKs
	if (GC.isEquipmentType(eYieldProducedType, EQUIPMENT_ARMOR_HORSES) && !isNative())
	{
		return 0;//XXX :P.
	}
    //tke
...
void CvCityAI::AI_assignDesiredYield()
{
	...
	if (isNative())
	{
		...
		for (int i = 0; i < NUM_YIELD_TYPES; ++i)
		{
            ...
            for (int iCivic = 0; iCivic < GC.getNumCivicInfos(); ++iCivic)
            {
                ...
                {
                    if (GC.getGameINLINE().getIdeasResearched((CivicTypes) iCivic) <= 0)
                    {
                        iValue = 0;
                        break;
                    }
    ...
    else
    {
		...
		for (int i = 0; i < NUM_YIELD_TYPES; ++i)
		{
            ...
            {
                if ((getRawYieldProduced(eYield) - getRawYieldConsumed(eYield)) < 1)
                {
                    iValue *= 10;
                }
            }
	...
	if (m_eDesiredYield != eBestYield)
	{
		...
		if (eBestYield != NO_YIELD)
		{
			...
			for (int iPlayer = 0; iPlayer < MAX_PLAYERS; ++iPlayer)
			{
				...
				if (kPlayer.isAlive() && kPlayer.getID() != getOwnerINLINE() && !GET_TEAM(getTeam()).isAtWar(kPlayer.getTeam()))
				{
					if (isScoutVisited(kPlayer.getTeam()) || (GET_TEAM(getTeam()).isHasMet(kPlayer.getTeam()) && plot()->getVisibilityCount(kPlayer.getTeam())))
					{
...
void CvCityAI::AI_updateRequiredYieldLevels()
{
		...
		if (!(kProfession.isCitizen() || kProfession.isWorkPlot()))
		{
			if (GC.getCivilizationInfo(getCivilizationType()).isValidProfession(eLoopProfession))
			{
				...
				for (int iI = 0; iI < NUM_YIELD_TYPES; ++iI)
				{
					...
					iRequired = std::max(iRequired, getMaxYieldCapacity(eYield) * iPercent / 100);
					iRequired = std::min(iRequired, getMaxYieldCapacity(eYield));
					///TKe
	...
	{
	    ///Tks Med
		aiLevels[YIELD_LUMBER] = std::max(aiLevels[YIELD_LUMBER], getMaxYieldCapacity(YIELD_LUMBER) / 2);
		///Tke
	}
