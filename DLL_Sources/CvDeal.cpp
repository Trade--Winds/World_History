...
//TK Update 1.1
//void CvDeal::kill(bool bKillTeam, TeamTypes eKillingTeam, bool bNoCancelMessage)
//{
//	if (!bNoCancelMessage && ((getLengthFirstTrades() > 0) || (getLengthSecondTrades() > 0)))
//	{
//	    ///TKe
//		CvWString szString;
...
void CvDeal::addTrades(CLinkList<TradeData>* pFirstList, CLinkList<TradeData>* pSecondList, bool bCheckAllowed)
{
	...
	if (bAlliance)
	{
		...
		{
		    GET_PLAYER(getFirstPlayer()).setVassalOwner(getSecondPlayer());
		}
		else if (GET_PLAYER(getSecondPlayer()).isNative() && !GET_PLAYER(getFirstPlayer()).isNative())
		{
		    GET_PLAYER(getSecondPlayer()).setVassalOwner(getFirstPlayer());
		}
		///Tke
	}
}
...
bool CvDeal::startTrade(TradeData trade, PlayerTypes eFromPlayer, PlayerTypes eToPlayer)
{
    ...
    case TRADE_RESEARCH:
    {
        CivicTypes eCivic = (CivicTypes)trade.m_iData1;
        if (eCivic != NO_CIVIC)
        {
            if (GET_PLAYER(eToPlayer).getCurrentResearch() != eCivic)
            {
                 GET_PLAYER(eToPlayer).setCurrentResearch(eCivic);
            }
            if (GET_PLAYER(eFromPlayer).getCurrentResearch() != eCivic)
            {
                GET_PLAYER(eFromPlayer).setCurrentResearch(eCivic);
            }
                GET_PLAYER(eToPlayer).setResearchPartner(eFromPlayer);
                GET_PLAYER(eFromPlayer).setResearchPartner(eToPlayer);


//            if (trade.m_iData1 == 0)
//            {
//                startTeamTrade(TRADE_RESEARCH, GET_PLAYER(eFromPlayer).getTeam(), GET_PLAYER(eToPlayer).getTeam(), true, trade.m_kTransport);
...
//            else
    ...
    case TRADE_IDEAS:
    {
        ...
        if (eCivic != NO_CIVIC && GET_PLAYER(eToPlayer).getCurrentResearch() == eCivic)
        {
            ...
            GET_PLAYER(eToPlayer).setCurrentResearch(NO_CIVIC);
            GET_PLAYER(eToPlayer).changeIdeasStored(-1);
	...
	case TRADE_GOLD:
		{
            ...
            ///Tks Med
            if (iGold >= GC.getDefineINT("TRADE_STIMULATES_RESEARCH_MIN_VALUE"))
            {
...
//                }
                ...
                {
                    GET_PLAYER(eToPlayer).changeIdeasStored(iExtraResearch);
                    GET_PLAYER(eToPlayer).changeIdeaProgress(eToPlayerResearch, iExtraResearch);
                    CvWString szBuffer = gDLL->getText("TXT_KEY_DEAL_STIMULATES_RESEARCH", iExtraResearch);
	...
	case TRADE_OPEN_BORDERS:
		if (trade.m_iData1 == 0)
		{
            ...
            for (int i = 0; i < GC.getNumFatherPointInfos(); ++i)
            {
                ...
                {
                    GET_TEAM(GET_PLAYER(eFromPlayer).getTeam()).changeFatherPoints(ePointType, GC.getDefineINT("DEAL_TRADE_RELATIONS_POINTS"));
                    GET_TEAM(GET_PLAYER(eToPlayer).getTeam()).changeFatherPoints(ePointType, GC.getDefineINT("DEAL_TRADE_RELATIONS_POINTS"));
                }
            }
			///TKe
		}
