...
bool CyPlayer::canMakeVassalDemand(int /*PlayerTypes*/ eVassal)
{
	return m_pPlayer ? m_pPlayer->canMakeVassalDemand((PlayerTypes)eVassal) : false;
}
///TKs Invention Core Mod v 1.0
bool CyPlayer::isProfessionValid(int /*ProfessionTypes*/ eProfession, int/*UnitTypes*/ eUnit)
{
    return m_pPlayer ? m_pPlayer->isProfessionValid((ProfessionTypes)eProfession, (UnitTypes) eUnit) : false;
}

int CyPlayer::getIdeasResearched(int /*CivicTypes*/ eCivic)
{
...
{
	return m_pPlayer ? m_pPlayer->getIdeasExperience() : -1;
}

int CyPlayer::getCurrentResearch() const
{
	return m_pPlayer ? m_pPlayer->getCurrentResearch() : -1;
}

void CyPlayer::setCurrentResearch(CivicTypes eCurrentResearch)
{
    if (m_pPlayer)
		m_pPlayer->setCurrentResearch((CivicTypes)eCurrentResearch);
}
int CyPlayer::getCurrentTradeResearch() const
{
	return m_pPlayer ? m_pPlayer->getCurrentTradeResearch() : -1;
}
void CyPlayer::setCurrentTradeResearch(CivicTypes eCurrentResearch)
{
    if (m_pPlayer)
		m_pPlayer->setCurrentTradeResearch((CivicTypes)eCurrentResearch);
}
int CyPlayer::getResearchPartner() const
{
	return m_pPlayer ? m_pPlayer->getResearchPartner() : -1;
}
int CyPlayer::getVassalOwner() const
{
	return m_pPlayer ? m_pPlayer->getVassalOwner() : -1;
}
int CyPlayer::getCensureType(int /*CensureType*/ eCensure)
{
   return m_pPlayer ? m_pPlayer->getCensureType((CensureType)eCensure) : -1;
}

int CyPlayer::getCurrentResearchProgress(bool bGetTurns, int /*CivicTypes*/ eCivic)
{
   return m_pPlayer ? m_pPlayer->getCurrentResearchProgress(bGetTurns, (CivicTypes)eCivic) : -1;
}
int CyPlayer::getIdeaProgress(int /*CivicTypes*/ eCivic)
{
   return m_pPlayer ? m_pPlayer->getIdeaProgress((CivicTypes)eCivic) : -1;
}
int CyPlayer::getCostToResearch(int /*CivicTypes*/ eCivic)
{
...
{
	return m_pPlayer ? m_pPlayer->getAttackForceKey() : std::wstring();
}
std::wstring CyPlayer::getDeclareKey( )
{
	return m_pPlayer ? m_pPlayer->getDeclareKey() : std::wstring();
}
std::wstring CyPlayer::getDawnKey( )
{
