...
void CvTeam::reset(TeamTypes eID, bool bConstructorCall)
{
	...
	m_iNumCities = 0;
	m_iTotalLand = 0;
	m_iMapTradingCount = GC.getXMLval(XML_ENABLE_MAP_TRADING);
	m_iGoldTradingCount = GC.getXMLval(XML_ENABLE_GOLD_TRADING);
	m_iOpenBordersTradingCount = GC.getXMLval(XML_ENABLE_OPEN_BORDERS);
	m_iDefensivePactTradingCount = GC.getXMLval(XML_ENABLE_DEFENSIVE_PACT_TRADING);
	m_iPermanentAllianceTradingCount = GC.getXMLval(XML_ENABLE_ALLIANCE_TRADING);
...
//        if (GET_PLAYER((PlayerTypes)iI).isAlive())
//        {
//            if (GET_PLAYER((PlayerTypes)iI).getTeam() == eTeam || GET_PLAYER((PlayerTypes)iI).getTeam() == getID())
//            {
//                if (GC.getGameINLINE().isBarbarianPlayer(GET_PLAYER((PlayerTypes)iI).getID()))
//                {
//                    return false;
//                }
//            }
...
//	///TKe
	...
	{
		if (GC.getGameINLINE().getElapsedGameTurns() < GC.getXMLval(XML_COLONIAL_FORCED_PEACE_TURNS))
		{
			return false;
		}
	}
		...
		if (GC.getGameINLINE().isFinalInitialized() && !(gDLL->GetWorldBuilderMode()))
		{
			...
			for (iI = 0; iI < MAX_PLAYERS; iI++)
			{
				...
				if (kLoopPlayer.isAlive())
				{
					...
					{
					    ///TKs Med
					    if (GET_PLAYER(kLoopPlayer.getID()).getVassalOwner() == NO_PLAYER)
					    {
...
int CvTeam::getFatherCostModifier() const
{
	...
	iCostModifier *= std::max(0, ((GC.getXMLval(XML_FATHER_COST_EXTRA_TEAM_MEMBER_MODIFIER) * (getNumMembers() - 1)) + 100));
	iCostModifier /= 100;
...
HandicapTypes CvTeam::getHandicapType() const
{
	...
	{
		return ((HandicapTypes)(GC.getXMLval(XML_STANDARD_HANDICAP)));
	}
}
	...
	if (!isHasMet(eIndex))
	{
		...
		if (GET_TEAM(eIndex).hasNativePlayer())
		{
			for (int i = 0; i < GC.getNumFatherPointInfos(); ++i)
			{
				...
				changeFatherPoints(ePointType, GC.getFatherPointInfo(ePointType).getMeetNativePoints() * getBestFatherPointMultiplier() / 100);
			}
		}
...
bool CvTeam::canDoRevolution() const
{
    ...
    ///Tke
	if (getRebelPercent() < GC.getXMLval(XML_REBEL_PERCENT_FOR_REVOLUTION))
	{
		return false;
	}
...
void CvTeam::doRevolution()
{
    ...
    for (int iCivic = 0; iCivic < GC.getNumCivicInfos(); ++iCivic)
    {
        if (GC.getCivicInfo((CivicTypes) iCivic).getCivicOptionType() == (CivicOptionTypes)GC.getXMLval(XML_CIVICOPTION_INVENTIONS))
        {
            ...
            {
                if (GET_PLAYER(getLeaderID()).getIdeasResearched((CivicTypes) iCivic) > 0)
                {
                    bHasConstitution = true;
                    break;
                }
            }
        }
