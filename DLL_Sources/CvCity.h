...
public:
	...
	///TKs Med
	void calculateNetYields(int aiYields[], int* aiProducedYields = NULL, int* aiConsumedYields = NULL, bool bPrintWarning = false, SpecialBuildingTypes eSpecialBuilding = NO_SPECIALBUILDING) const;
	///Tke
	int calculateNetYield(YieldTypes eYield) const;
	///TKs Med
	void setTradePostBuilt(TeamTypes eTeam, bool bBuilt);
	DllExport bool isTradePostBuilt(TeamTypes eTeam) const;
	int calculateActualYieldProduced(YieldTypes eYield, SpecialBuildingTypes eSpecialBuilding = NO_SPECIALBUILDING) const;
	///TKe
	...
	int getMaxYieldCapacity(YieldTypes eYield = NO_YIELD) const;
	...
	bool removePopulationUnit(CvUnit* pUnit, bool bDelete, ProfessionTypes eProfession);
	...
	///TKs Med
	int getEventTimer(int iEvent) const;
	void changeEventTimer(int iEvent, int iChange);
	int getYieldAutoSellPercent(YieldTypes eYield) const;
	BuildingTypes getBestFreeBuilding();
	BuildingTypes getFreeYieldBuilding(YieldTypes eYield) const;
	YieldTypes getSelectedArmor() const;
	void setSelectedArmor(YieldTypes eArmor);
	void setCityType(MedCityTypes eType);
	void setMaxCityPop(int iPop);
...
{
	return getBuildingYieldDemand(eYield) / 100;
}
// domestic yield demand - end - Nightinggale
