...
bool shouldMoveBefore(const CvUnit* pUnitA, const CvUnit* pUnitB)
{
    ...
    {
        FAssert(pUnitA == NULL || pUnitB == NULL);
        return false;
    }
    ///TKe
...
bool PUF_isBarbarianEnemy(const CvUnit* pUnit, int iData1, int iData2)
{
	...
	if (eOurTeam == UNKNOWN_TEAM)
	{
	    eOurTeam = GET_PLAYER(pUnit->getOwner()).getTeam();
	}

	if (GC.getGameINLINE().isBarbarianPlayer(GET_TEAM(eOurTeam).getLeaderID()) || pUnit->isAlwaysHostile(pUnit->plot()))
	{
	    return true;
	}
...
int getTurnYearForGame(int iGameTurn, int iStartYear, CalendarTypes eCalendar, GameSpeedTypes eSpeed)
{
    ...
    if (iGameTurn >= 1 && GC.getDefineINT("USE_MEDIEVAL_CALENDER") > 0)
    {
        //iGameTurn -= 1;
        if (iGameTurn <= GC.getDefineINT("CALENDER_YEAR_FIRST_CHANGE_TURN"))
        {
            iGameYear += iGameTurn * GC.getDefineINT("CALENDER_YEAR_FIRST_MULTIPLIER");
        }
        else if (iGameTurn <= GC.getDefineINT("CALENDER_YEAR_FIRST_CHANGE_TURN") * 2)
        {
            int iFirstIncrement = (GC.getDefineINT("CALENDER_YEAR_FIRST_CHANGE_TURN") * GC.getDefineINT("CALENDER_YEAR_FIRST_MULTIPLIER"));
            iGameYear +=  iFirstIncrement + ((iGameTurn - (GC.getDefineINT("CALENDER_YEAR_FIRST_CHANGE_TURN"))) * GC.getDefineINT("CALENDER_YEAR_SECOND_MULTIPLIER"));
        }
        else if (iGameTurn <= GC.getDefineINT("CALENDER_YEAR_SECOND_CHANGE_TURN"))
        {
            int iFirstIncrement = (GC.getDefineINT("CALENDER_YEAR_FIRST_CHANGE_TURN") * GC.getDefineINT("CALENDER_YEAR_FIRST_MULTIPLIER"));
            int iSecondIncrement = (GC.getDefineINT("CALENDER_YEAR_FIRST_CHANGE_TURN")) * GC.getDefineINT("CALENDER_YEAR_SECOND_MULTIPLIER");
            iGameYear += iFirstIncrement + iSecondIncrement + ((iGameTurn - (GC.getDefineINT("CALENDER_YEAR_FIRST_CHANGE_TURN") * 2)) * GC.getDefineINT("CALENDER_YEAR_THIRD_MULTIPLIER"));
        }
        else if (iGameTurn > GC.getDefineINT("CALENDER_YEAR_SECOND_CHANGE_TURN"))
        {
            int iFirstIncrement = GC.getDefineINT("CALENDER_YEAR_FIRST_CHANGE_TURN") * GC.getDefineINT("CALENDER_YEAR_FIRST_MULTIPLIER");
            int iSecondIncrement = GC.getDefineINT("CALENDER_YEAR_FIRST_CHANGE_TURN") * GC.getDefineINT("CALENDER_YEAR_SECOND_MULTIPLIER");
            int iThirdIncrement = GC.getDefineINT("CALENDER_YEAR_FIRST_CHANGE_TURN") * 2 * GC.getDefineINT("CALENDER_YEAR_THIRD_MULTIPLIER");
            iGameYear += iFirstIncrement + iSecondIncrement + iThirdIncrement + (iGameTurn - GC.getDefineINT("CALENDER_YEAR_SECOND_CHANGE_TURN"));
            ++iGameYear;
        }
        return iGameYear;
    }
    ...
    {
        iGameYear = getTurnMonthForGame(iGameTurn, iStartYear, eCalendar, eSpeed) / GC.getNumMonthInfos();
    }
    return iGameYear;
    ///Tke
}
...
void getMissionAIString(CvWString& szString, MissionAITypes eMissionAI)
{
	...
	///TKs Med
	case MISSIONAI_PILGRAM: szString = L"MISSIONAI_PILGRAM"; break;
	///TKe
