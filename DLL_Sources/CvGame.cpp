...
void CvGame::createBarbarianPlayer()
{
    if (getBarbarianPlayer() == NO_PLAYER)
    {
        ...
        for (int iP=0; iP < MAX_PLAYERS; iP++)
        {
            ...
            if (player.isAlive())
            {
                if (player.getCivilizationType() == (CivilizationTypes) GC.getDefineINT("BARBARIAN_CIVILIZATION"))
                {
...
//                        }
						...
						for (iJ = 0; iJ < GC.getNumUnitInfos(); iJ++)
						{
                            ...
                            {
                                iRand = GC.getDefineINT("WILD_ANIMAL_LAND_UNIT_VARIATION_WEIGHT");
					...
					if (pPlot != NULL)
					{
					    ...
					    */
						...
						for (iJ = 0; iJ < GC.getNumUnitInfos(); iJ++)
						{
                            ...
                            if (eLastUnit != NO_UNIT && eLastUnit == eBestUnit)
                            {
                                iRand = GC.getDefineINT("WILD_ANIMAL_SEA_UNIT_VARIATION_WEIGHT");
...
void CvGame::createVassalPlayer(PlayerTypes eVassalOwner, CvCity* pCity)
{
        ...
        if (eNewPlayer != NO_PLAYER)
        {
            LeaderHeadTypes eBarbLeader = (LeaderHeadTypes) GC.getDefineINT("VASSAL_LEADER");
            CivilizationTypes eBarbCiv = (CivilizationTypes) GC.getDefineINT("VASSAL_CIVILIZATION");
            if (eBarbLeader != NO_LEADER && eBarbCiv != NO_CIVILIZATION)
            {
                addPlayer(eNewPlayer, eBarbLeader, eBarbCiv);
                //setBarbarianPlayer(eNewPlayer);
