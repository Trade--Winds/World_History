...
bool CvTeamAI::AI_shareWar(TeamTypes eTeam) const
{
	...
	for (iI = 0; iI < MAX_TEAMS; iI++)
	{
		...
		{
		     ///TKs Med
            if (GC.getGameINLINE().isBarbarianPlayer(GET_TEAM((TeamTypes)iI).getLeaderID()))
            {
                continue;
            }
            ///Tke
...
///Tks Med
	...
	for (iI = 0; iI < MAX_PLAYERS; iI++)
	{
		if (GET_PLAYER((PlayerTypes)iI).isAlive())
		{
			if (GET_PLAYER((PlayerTypes)iI).getTeam() == getID())
			{
				...
				if ((eAttitude + 1) == GC.getLeaderHeadInfo(GET_PLAYER((PlayerTypes)iI).getPersonalityType()).getDefensivePactRefuseAttitudeThreshold())
				{
					if (getPower() > (GET_TEAM(eTeam).getPower() * 2))
                    {
	...
	if (AI_getAtPeaceCounter(eTeam) < GC.getDefineINT("ALLIANCE_PACT_PEACE_DENIAL"))
	{
		return DENIAL_ATTITUDE;
	}
///TKe
...
DenialTypes CvTeamAI::AI_permanentAllianceTrade(TeamTypes eTeam) const
{
	...
	if (AI_getDefensivePactCounter(eTeam) + AI_getAtPeaceCounter(eTeam) < GC.getDefineINT("ALLIANCE_PACT_PEACE_DENIAL"))
	{
	...
	for (iI = 0; iI < MAX_PLAYERS; iI++)
	{
		if (GET_PLAYER((PlayerTypes)iI).isAlive())
		{
			if (GET_PLAYER((PlayerTypes)iI).getTeam() == getID())
			{
			    iOurCulture += GET_PLAYER((PlayerTypes)iI).countTotalCulture();
			    iOurCulture += GET_PLAYER((PlayerTypes)iI).getTotalLand();
			}
			else if (GET_PLAYER((PlayerTypes)iI).getTeam() == GET_TEAM(eTeam).getID())
			{
			    iTheirCulture += GET_PLAYER((PlayerTypes)iI).countTotalCulture();
			    iTheirCulture += GET_PLAYER((PlayerTypes)iI).getTotalLand();
			}
		}
	}
    iTheirCulture = iTheirCulture * GC.getDefineINT("ALLIANCE_CULTURE_PERCENT_DENIAL") / 100;
...
DenialTypes CvTeamAI::AI_collaborateResearchTrade(TeamTypes eTeam) const
{
	AttitudeTypes eAttitude;
	int iI;
...
//	}
    ...
    if (GET_PLAYER(getLeaderID()).getCurrentResearch() != NO_CIVIC)
    {

        int iMyTurns = GET_PLAYER(getLeaderID()).getTurnstoCompleteResearch(false);
        if (iMyTurns > 0 && iMyTurns <= GC.getDefineINT("TK_RESEARCH_PACT_IDEAS_THRESHOLD") / 3)
        {
            return DENIAL_NO_GAIN;
        }
        int iResearch = GET_PLAYER(GET_TEAM(eTeam).getLeaderID()).getTurnstoCompleteResearch(true);
        int iCost = GET_PLAYER(getLeaderID()).getCostToResearch(GET_PLAYER(getLeaderID()).getCurrentResearch());
        if (iResearch == 0)
        {
            return DENIAL_NO_GAIN;
        }
        if ((100 * iResearch) / iCost < GC.getDefineINT("TK_RESEARCH_PACT_IDEAS_THRESHOLD"))
        {
            return DENIAL_NO_GAIN;
        }
    }
