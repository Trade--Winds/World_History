...
void CvDLLWidgetData::parseHelp(CvWStringBuffer &szBuffer, CvWidgetDataStruct &widgetDataStruct)
{
	...
	case WIDGET_TAX_ADVISOR:
        szBuffer.append(L"Ajust Gold, Missinary Rate, and Tax Rate");
		break;
    //Tke
    ...
    case WIDGET_RECEIVE_MOVE_CARGO_TO_TRANSPORT:
        if (widgetDataStruct.m_iData1 != -1)
		{
		    ...
		    if (pUnit != NULL && pCity != NULL)
			{
			    ...
			    {
                    CvPopupInfo* pInfo = new CvPopupInfo(BUTTONPOPUP_CHOOSE_PROFESSION, pCity->getID(), pUnit->getID());
...
void CvDLLWidgetData::parseCityNameHelp(CvWidgetDataStruct &widgetDataStruct, CvWStringBuffer &szBuffer)
{
	...
	if (pHeadSelectedCity != NULL)
	{
		...
		int iPopulation = pHeadSelectedCity->getPopulation();
        int iMaxPop = pHeadSelectedCity->getMaxCityPop();
        if (iPopulation != iMaxPop)
        {
            if (iMaxPop < GC.getDefineINT("MAX_CITY_POPULATION_COMMUNE"))
            {
                szBuffer.append(NEWLINE);
        ...
        if (iPopulation == iMaxPop)
        {
            ...
            {
                BuildingTypes eBuilding = (BuildingTypes) GC.getCivilizationInfo(pHeadSelectedCity->getCivilizationType()).getCivilizationBuildings(iBuildingClass);
                if (eBuilding != NO_BUILDING)
                {
                    if (GC.getBuildingInfo(eBuilding).isIncreasesCityPopulation() != 0)
                    {
                        if (!pHeadSelectedCity->isHasBuilding(eBuilding))
                        {
                            if (pHeadSelectedCity->canConstruct(eBuilding, true, false, true))
                            {
                                iFoundBCount++;
                                if (iFoundBCount > 1)
                                {
                                ...
                                szBuffer.append(gDLL->getText("TXT_KEY_ACTION_CANNOT_JOIN_OVERPOPULATED_BUILDING_LIST", GC.getBuildingInfo(eBuilding).getDescription()));
                                ...
                                {	CvCivicInfo& kCivicInfo = GC.getCivicInfo((CivicTypes) iCivic);
                                    if (kCivicInfo.getCivicOptionType() == (CivicOptionTypes)GC.getDefineINT("CIVICOPTION_INVENTIONS"))
                                    {
                                       ...
                                       // {
                                            ...
                                            {
                                                if (GET_PLAYER(pHeadSelectedCity->getOwner()).getIdeasResearched((CivicTypes) iCivic) == 0)
                                                {
                                                    iFoundBCount++;
                                                    if (iFoundBCount > 1)
                                                    {
                                                    ...
                                                    szBuffer.append(gDLL->getText("TXT_KEY_ACTION_CANNOT_JOIN_OVERPOPULATED_BUILDING_RESEARCH_LIST", GC.getBuildingInfo(eBuilding).getDescription()));
                                                }
                                            }
                                       // }
                                    }
	...
	if (pHeadSelectedUnit != NULL)
	{
		if (GC.getActionInfo(widgetDataStruct.m_iData1).getMissionType() != NO_MISSION)
		{
			...
			else if (GC.getActionInfo(widgetDataStruct.m_iData1).getMissionType() == MISSION_FOUND)
			{
			    ///TKs Med
                int iCityType = 0;
                if (pHeadSelectedUnit->getProfession() != NO_PROFESSION)
                {
                    iCityType = GC.getProfessionInfo(pHeadSelectedUnit->getProfession()).getFoundCityType();
                }
				if (!(GET_PLAYER(pHeadSelectedUnit->getOwnerINLINE()).canFound(pMissionPlot->getX_INLINE(), pMissionPlot->getY_INLINE(), false, iCityType)))
				{
				    ///TKe
					bValid = true;
                            ...
                            for (int iBuildingClass = 0; iBuildingClass < GC.getNumBuildingClassInfos(); ++iBuildingClass)
                            {
                                ...
                                if (eBuilding != NO_BUILDING)
                                {
                                    if (GC.getBuildingInfo(eBuilding).isIncreasesCityPopulation() != 0)
                                    {
                                        ...
                                        {
                                            if (pCity->canConstruct(eBuilding, true, false, true))
                                            {
                                                iFoundBCount++;
                                                if (iFoundBCount > 1)
                                                {
                                            ...
                                            else
                                            {
                                                ...
                                                {	CvCivicInfo& kCivicInfo = GC.getCivicInfo((CivicTypes) iCivic);
                                                    if (kCivicInfo.getCivicOptionType() == (CivicOptionTypes)GC.getDefineINT("CIVICOPTION_INVENTIONS"))
                                                    {
                                                        if (kCivicInfo.getAllowsBuildingTypes(GC.getBuildingInfo(eBuilding).getBuildingClassType()) > 0)
                                                        {
                                                            if (GET_PLAYER(pCity->getOwner()).getIdeasResearched((CivicTypes) iCivic) == 0)
                                                            {
                                                                iFoundBCount++;
                                                                if (iFoundBCount > 1)
                                                                {
			...
			else if (GC.getActionInfo(widgetDataStruct.m_iData1).getMissionType() == MISSION_BUILD)
			{
				...
				///Tks Med
				int iNotBuild = 0;
				int iCanNotBuildCode = 0;
				bool bRange = false;
				while (pSelectedUnitNode != NULL)
				{
					pSelectedUnit = ::getUnit(pSelectedUnitNode->m_data);
                    iNotBuild = pSelectedUnit->canBuildInt(pMissionPlot, eBuild);
					if (iNotBuild == -1)
					{
						bValid = true;
						break;
					...
					{
						 iCanNotBuildCode = iNotBuild;
					}
                    ///Tke
					pSelectedUnitNode = gDLL->getInterfaceIFace()->nextSelectionListNode(pSelectedUnitNode);
				}

				if (!bValid && !pSelectedUnit->plot()->isPeak())
				{
					if (eBuildImprovement != NO_IMPROVEMENT)
					{
						...
						{
                            szBuffer.append(NEWLINE);
                            szBuffer.append(gDLL->getText("TXT_KEY_ACTION_CANNOT_BUILD_RANGE", GC.getMIN_CITY_RANGE()));
						}
                        else if (iCanNotBuildCode == 2)
                        {
                            for (int i = 0; i < NUM_YIELD_TYPES; ++i)
                            {
                                YieldTypes eYield = (YieldTypes) i;

                                int iRequiredYield = GC.getImprovementInfo(eBuildImprovement).getRequiredCityYields(eYield);
                                if (iRequiredYield > 0)
                                {
                                    szBuffer.append(NEWLINE);
                                    szBuffer.append(gDLL->getText("TXT_KEY_ACTION_NEEDS_TRANSPORTED_YIELDS", iRequiredYield, GC.getYieldInfo(eYield).getChar()));
                                }

                            }
                        }
                        else if (iCanNotBuildCode == 1)
                        {
                            BonusTypes eBonus = (BonusTypes)GC.getImprovementInfo(eBuildImprovement).getBonusCreated();                        //if (bCityYields)
                            if (eBonus != NO_BONUS)
                            {
                                szBuffer.append(NEWLINE);
                                szBuffer.append(gDLL->getText("TXT_KEY_ACTION_NEEDS_BONUS_YIELDS", GC.getBonusInfo(eBonus).getChar()));
                                ...
                                for (int i = 0; i < NUM_YIELD_TYPES; ++i)
                                {
                                    ...
                                    if (iRequiredYield > 0)
                                    {
                                        ...
                                        {
                                            szBuffer.append(NEWLINE);
                                            szBuffer.append(gDLL->getText("TXT_KEY_ACTION_NEEDS_CITY_YIELDS", iRequiredYield, GC.getYieldInfo(eYield).getChar(), pCity->getNameKey()));
                                        }
                                    }
				    ...
				    if (bValid)
                    {
                        if (GC.getImprovementInfo(eBuildImprovement).isOutsideBorders())
                        {
                            for (int i = 0; i < NUM_YIELD_TYPES; ++i)
                            {
                                ...
                                {
                                    szBuffer.append(NEWLINE);
                                    szBuffer.append(gDLL->getText("TXT_KEY_ACTION_NEEDS_TRANSPORTED_YIELDS_CONSUMED", iRequiredYield, GC.getYieldInfo(eYield).getChar()));

                                }

                            }
                        }
                        BonusTypes eBonus = (BonusTypes)GC.getImprovementInfo(eBuildImprovement).getBonusCreated();
                        if (eBonus != NO_BONUS)
                        {
                            bool bBonus = false;
                            if (pMissionPlot->getBonusType() == eBonus)
                            {
                                bBonus = true;
                                //szBuffer.append(NEWLINE);
                                //szBuffer.append(gDLL->getText("TXT_KEY_ACTION_BONUS_YIELDS", GC.getBonusInfo(eBonus).getChar()));
                            }
                            if (!bBonus)
                            {
                                for (int i = 0; i < NUM_YIELD_TYPES; ++i)
                                {
                                    ...
                                    if (iRequiredYield > 0)
                                    {
                                        ...
                                        {
                                            szBuffer.append(NEWLINE);
                                            szBuffer.append(gDLL->getText("TXT_KEY_ACTION_NEEDS_CITY_YIELDS_CONSUMED", iRequiredYield, GC.getYieldInfo(eYield).getChar(), pCity->getNameKey()));
                                        }
                                    }
		...
		if (GC.getActionInfo(widgetDataStruct.m_iData1).getCommandType() != NO_COMMAND)
		{
			...
			else if (GC.getActionInfo(widgetDataStruct.m_iData1).getCommandType() == COMMAND_GIFT)
			{
			 ...
			 ///Tks Return Home
            else if (GC.getActionInfo(widgetDataStruct.m_iData1).getCommandType() == COMMAND_ASSIGN_HOME_CITY)
			{
                    ...
                    {
                        pSelectedUnit = ::getUnit(pSelectedUnitNode->m_data);
                        if (pSelectedUnit->getDomainType() == DOMAIN_SEA || pSelectedUnit->cargoSpace() > 0 || GC.getUnitInfo(pSelectedUnit->getUnitType()).isTreasure())
                        {
                ...
                {
                    if(pCommandUnit->getHomeCity() != NULL)
                    {
                        szBuffer.append(NEWLINE);
						szBuffer.append(gDLL->getText("TXT_KEY_NO_ACTION_HAS_HOME_CITY", pCommandUnit->getHomeCity()->getName().GetCString()));
                    }
                }
            ...
            ///TKe
			///TKs Med
			else if (GC.getActionInfo(widgetDataStruct.m_iData1).getCommandType() == COMMAND_KING_TRANSPORT)
			{
				...
				if (pSelectedUnitNode != NULL)
				{
				   ...
				   if (pSelectedUnit != NULL && !pSelectedUnit->canKingTransport())
				   {
				       CvPlot* pPlot = pSelectedUnit->plot();
				       CvCity* pCity = pPlot->getPlotCity();
                        if (pCity != NULL)
                        {
                            if (pPlot->getTeam() != pSelectedUnit->getTeam())
                            {
                                szBuffer.append(NEWLINE);
                                szBuffer.append(gDLL->getText("TXT_KEY_TREASURE_REQUIRES_CITY"));
                            }
                        }
                        else
                        {
                        ...
                        if (GC.getUnitInfo(pSelectedUnit->getUnitType()).getConvertsToBuildingClass() != NO_BUILDINGCLASS)
                        {
                            //eGiftPlayer = pHeadSelectedUnit->getOwnerINLINE();
                            BuildingTypes eBuilding = ((BuildingTypes)(GC.getCivilizationInfo(pSelectedUnit->getCivilizationType()).getCivilizationBuildings((BuildingClassTypes)GC.getUnitInfo(pSelectedUnit->getUnitType()).getConvertsToBuildingClass())));
                            if (pCity != NULL)
                            {
                                if (GC.getBuildingInfo(eBuilding).getCityType() > -1 && pCity->getCityType() != (MedCityTypes)GC.getBuildingInfo(eBuilding).getCityType())
                                {
                                    szBuffer.append(NEWLINE);
                                    szBuffer.append(gDLL->getText("TXT_KEY_TREASURE_REQUIRES_BUILDING", GC.getBuildingInfo(eBuilding).getDescription()));
                                }
                            }
                            else
                            {
			...
			else if (GC.getActionInfo(widgetDataStruct.m_iData1).getCommandType() == COMMAND_BUILD_TRADINGPOST)
			{
			    CvUnit* pTrader = NULL;
				pSelectedUnitNode = gDLL->getInterfaceIFace()->headSelectionListNode();
				while (pSelectedUnitNode != NULL)
				{
					pSelectedUnit = ::getUnit(pSelectedUnitNode->m_data);
					if (pSelectedUnit->canBuildTradingPost(true))
					{
				...
				if (pTrader != NULL)
				{
                    ...
                    //{
                        szBuffer.append(NEWLINE);
                        szBuffer.append(gDLL->getText("TXT_KEY_ESTABLISH_TRADEPOST_GOLD", GC.getDefineINT("ESTABLISH_TRADEPOST_COST")));
                    //}
				}
			}
			///Tke
...
void CvDLLWidgetData::parseContactCivHelp(CvWidgetDataStruct &widgetDataStruct, CvWStringBuffer &szBuffer)
{
	...
	else
	{
		if (!otherPlayer.isHuman())
		{
			...
			///TKs Invention Core Mod v 1.0
            CvWString szTechList;
			if (otherPlayer.AI_isWillingToTalk(GC.getGameINLINE().getActivePlayer()))
			{
                for (int iLoopCivic = 0; iLoopCivic < GC.getNumCivicInfos(); ++iLoopCivic)
                {
                    if (otherPlayer.getIdeasResearched((CivicTypes)iLoopCivic) > 0 && GET_PLAYER(GC.getGameINLINE().getActivePlayer()).getIdeasResearched((CivicTypes)iLoopCivic) == 0)
                    {
                        if (!GC.getCivicInfo((CivicTypes)iLoopCivic).isNoneTradeable())
                        {
                            if (!szTechList.empty())
                            {
                                szTechList += L", ";
                            ...
                            szTechList += CvWString::format(L"%s", GC.getCivicInfo((CivicTypes)iLoopCivic).getDescription());
                        }
                    }
                }
                if (!szTechList.empty())
                {
                    szBuffer.append(NEWLINE);
                    szBuffer.append(gDLL->getText("TXT_KEY_TRADEABLE_KNOWLEDGE", szTechList.GetCString()));
...
void CvDLLWidgetData::parseTradeItem(CvWidgetDataStruct &widgetDataStruct, CvWStringBuffer &szBuffer)
{
	...
	if ((eWhoFrom != NO_PLAYER) && (eWhoTo != NO_PLAYER))
	{
	    ...
	    case TRADE_RESEARCH:
	    {
	        szBuffer.append(gDLL->getText("TXT_KEY_TRADE_CURRENT_RESEARCH_TEXT"));
            CivicTypes eCivic = (CivicTypes) widgetDataStruct.m_iData2;
            if (eCivic != NO_CIVIC)
            {
                CvWStringBuffer szTBuffer;

                GAMETEXT.parseCivicInfo(szTBuffer, eCivic, false, true, false);
                szBuffer.append(NEWLINE);
                szBuffer.append(szTBuffer);
            }
            //szBuffer = gDLL->getText("TXT_KEY_TRADE_CURRENT_RESEARCH_TEXT") + szBuffer;
...
void CvDLLWidgetData::parsePopulationHelp(CvWidgetDataStruct &widgetDataStruct, CvWStringBuffer &szBuffer)
{
	...
	if (pHeadSelectedCity != NULL)
	{
		...
		szBuffer.assign(gDLL->getText("TXT_KEY_MISC_LUXURY_FOOD_THRESHOLD", pHeadSelectedCity->getFood(), pHeadSelectedCity->growthThreshold(), pHeadSelectedCity->getYieldStored(YIELD_GRAIN), GC.getDefineINT("BASE_CITY_LUXURY_FOOD_THRESHOLD_MOD"), GC.getYieldInfo(YIELD_GRAIN).getChar()));
		///Tke
	}
}
...
void CvDLLWidgetData::parseProfessionHelp(CvWidgetDataStruct &widgetDataStruct, CvWStringBuffer &szBuffer)
{
	if (widgetDataStruct.m_iData2 != 0)
	{
		GAMETEXT.setProfessionHelp(szBuffer, (ProfessionTypes)widgetDataStruct.m_iData1, false, widgetDataStruct.m_iData2);
	}
}
...
void CvDLLWidgetData::parsePromotionHelp(CvWidgetDataStruct &widgetDataStruct, CvWStringBuffer &szBuffer)
{
	...
	if (widgetDataStruct.m_iData2 != 0)
	{
		...
		GAMETEXT.setPromotionHelp(szBuffer, (PromotionTypes)widgetDataStruct.m_iData1, bPedia);
	}
	///TKe
}
...
void CvDLLWidgetData::parseCityYieldHelp(CvWidgetDataStruct &widgetDataStruct, CvWStringBuffer &szBuffer)
{
    ...
    {
        pCity = kPlayer.getCity(widgetDataStruct.m_iData2);
    }
	if (NULL == pCity)
	{
		pCity = gDLL->getInterfaceIFace()->getHeadSelectedCity();
	...
	if (NULL != pCity)
	{
		GAMETEXT.setYieldHelp(szBuffer, *pCity, eYield, bSelectArmor);
	}
	///TKe
	else
	{
	...
	if (NULL == pCity)
	{
        ...
        if (eUnitClass != NO_UNITCLASS)
        {
            ...
            if (eUnit != NO_UNIT)
            {
                if (GC.getUnitInfo(eUnit).getRequiredTransportSize() > 1)
                {
                    szBuffer.append(NEWLINE);
...
///TKs Invention Core Mod v 1.0
void CvDLLWidgetData::doInventorsHouse(const CvWidgetDataStruct& widgetDataStruct)
{
    if (GET_PLAYER(GC.getGameINLINE().getActivePlayer()).getResearchPartner() != NO_PLAYER)
    {
        return;
    }

    if (widgetDataStruct.m_iData1 >= 99)
    {
        if (GET_PLAYER(GC.getGameINLINE().getActivePlayer()).canDoCivics((CivicTypes)widgetDataStruct.m_iData2))
        {
            GET_PLAYER(GC.getGameINLINE().getActivePlayer()).setCurrentResearch((CivicTypes)widgetDataStruct.m_iData2);
        }

        return;
    }

//    if (widgetDataStruct.m_iData1 >= 69 && widgetDataStruct.m_iData1 < 99)
//    {
//        if (GET_PLAYER(GC.getGameINLINE().getActivePlayer()).canDoCivics((CivicTypes)widgetDataStruct.m_iData2))
//        {
//            GET_PLAYER(GC.getGameINLINE().getActivePlayer()).setCurrentTradeResearch((CivicTypes)widgetDataStruct.m_iData2);
//        }
//
//        return;
//    }
	...
	if (pHeadSelectedCity != NULL && pHeadSelectedCity->getOwnerINLINE() == GC.getGameINLINE().getActivePlayer())
	{
		...
		CvPopupInfo* pInfo = new CvPopupInfo(BUTTONPOPUP_CHOOSE_INVENTION, (CivicOptionTypes)GC.getDefineINT("CIVICOPTION_INVENTIONS"),  pHeadSelectedCity->getID());
		gDLL->getInterfaceIFace()->addPopup(pInfo, NO_PLAYER, true);
	}
	else
	{
	    CvPopupInfo* pInfo = new CvPopupInfo(BUTTONPOPUP_CHOOSE_INVENTION, (CivicOptionTypes)GC.getDefineINT("CIVICOPTION_INVENTIONS"), 1);
		gDLL->getInterfaceIFace()->addPopup(pInfo, NO_PLAYER, true);
	}

}

void CvDLLWidgetData::doArmorsmith(const CvWidgetDataStruct& widgetDataStruct)
{
	CvCity* pHeadSelectedCity = gDLL->getInterfaceIFace()->getHeadSelectedCity();
	if (pHeadSelectedCity != NULL && pHeadSelectedCity->getOwnerINLINE() == GC.getGameINLINE().getActivePlayer())
	{
		CvPopupInfo* pInfo = new CvPopupInfo(BUTTONPOPUP_ARMORSMITH, gDLL->getInterfaceIFace()->getHeadSelectedCity()->getID());
		gDLL->getInterfaceIFace()->addPopup(pInfo, NO_PLAYER, true);
	}
}
void CvDLLWidgetData::doMarket(const CvWidgetDataStruct& widgetDataStruct)
{
	CvCity* pHeadSelectedCity = gDLL->getInterfaceIFace()->getHeadSelectedCity();
	if (pHeadSelectedCity != NULL && pHeadSelectedCity->getOwnerINLINE() == GC.getGameINLINE().getActivePlayer())
	{
...
void CvDLLWidgetData::parseInventorsHouseHelp(CvWidgetDataStruct& widgetDataStruct, CvWStringBuffer& szBuffer)
{
    ...
    if (GET_PLAYER(GC.getGameINLINE().getActivePlayer()).getCurrentResearch() != NO_CIVIC)
    {

        CivicTypes eCivic = (CivicTypes)GET_PLAYER(GC.getGameINLINE().getActivePlayer()).getCurrentResearch();
        CvCivicInfo& kCurrentCivicInfo = GC.getCivicInfo(eCivic);
        if (GET_PLAYER(GC.getGameINLINE().getActivePlayer()).getResearchPartner() != NO_PLAYER)
        {
            szBuffer.append(gDLL->getText("TXT_KEY_CITY_CURRENT_RESEARCH_LOCKED"));
            szBuffer.append(NEWLINE);

        }

        szBuffer.append(gDLL->getText("TXT_KEY_CHOOSE_WIDGET_CURRENT_RESEARCH_PROGRESS", kCurrentCivicInfo.getDescription(), GET_PLAYER(GC.getGameINLINE().getActivePlayer()).getCurrentResearchProgress(true)));
    }
    else
    {
        szBuffer.append(gDLL->getText("TXT_KEY_NO_RESEARCH_MENU"));
    }

}
//TK Tax
void CvDLLWidgetData::doTaxAdvisor(const CvWidgetDataStruct& widgetDataStruct)
{
	CvCity* pHeadSelectedCity = gDLL->getInterfaceIFace()->getHeadSelectedCity();
	if (pHeadSelectedCity != NULL && pHeadSelectedCity->getOwnerINLINE() == GC.getGameINLINE().getActivePlayer())
	{
		CvPopupInfo* pInfo = new CvPopupInfo(BUTTONPOPUP_TAX_ADVISOR, 1, 1);
		gDLL->getInterfaceIFace()->addPopup(pInfo, NO_PLAYER, true);
	}
	else
	{
