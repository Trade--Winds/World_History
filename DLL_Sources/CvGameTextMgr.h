...
public:
	...
	DllExport void parsePromotionHelp(CvWStringBuffer &szBuffer, PromotionTypes ePromotion, const wchar* pcNewline = NEWLINE, bool bCivilopediaText=false);

	DllExport void parseCivicInfo(CvWStringBuffer &szBuffer, CivicTypes eCivic, bool bCivilopediaText = false, bool bPlayerContext = false, bool bSkipName = false, bool bOnlyCost = false, CivilizationTypes eCivilization = NO_CIVILIZATION);
	///TKe
	...
	///TKs Med
	DllExport void setYieldPriceHelp(CvWStringBuffer &szBuffer, PlayerTypes ePlayer, YieldTypes eYield, int iAmount=0);
	DllExport void setYieldHelp(CvWStringBuffer &szBuffer, CvCity& city, YieldTypes eYieldType, bool bSelectedArmor=false);
	///TKe
